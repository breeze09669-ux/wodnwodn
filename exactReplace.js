...
const endIdx = dataJs.indexOf('const mockRestaurants = [');

if (startIdx !== -1 && endIdx !== -1) {
  dataJs = dataJs.substring(0, startIdx) + newMockScripts + '\\n\\n' + dataJs.substring(endIdx);
  fs.writeFileSync('data.js', dataJs);
  console.log('Successfully replaced data.js mockScripts!');
} else {
  console.log('Could not find boundaries in data.js');
}
